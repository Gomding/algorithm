...
public class NUM4789 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[] clap;        // 기립박수의 조건 표
        int now;            // 현재 박수 치고 있는 사람
        int cnt;            // 고용해야하는 사람의 수

        for (int tc = 1; tc <= T; tc++) {

            clap = bf.readLine().toCharArray();

            now = 0;
            cnt = 0;

            if (clap[0] != '0') {       // 첫번째 숫자가 0이 아니라면 공연이 끝나고 조건없이 박수치는 사람의 숫자를 now 에 더해줌
                now = Character.getNumericValue(clap[0]);
            }

            for (int i = 1; i < clap.length; i++) {
                if (clap[i] != '0') {
                    if(now >= i)    // 현재 박수치고 있는 사람이 i번째의 조건보다 많거나 같을 때
                        now += Character.getNumericValue(clap[i]);  // 현재 박수치는 사람에 그 사람들의 수 만큼 더해줌
                    else {  // 현재 박수 치고 있는 사람의 숫자가 i 번째 의 조건보다 적을 때
                        int a = i - now;    // 현재 고용해야 할 사람 수를 계산
                        cnt += a;   // 고용해야할 사람 수에 더해줌
                        now += Character.getNumericValue(clap[i]) + a;  // 사람을 고용한 후 고용한 사람과 조건을 만족한 사람들은 현재 박수치는 사람들에 더해줌
                    }
                }
            }
